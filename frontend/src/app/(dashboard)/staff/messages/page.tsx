...
export default function StaffMessagesPage() {
  ...
  // Payment Confirmation Modal State in Messages
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      try {
        ...
        setCurrentUserEmail(parsedUser.email || "staff@etayo.gov.ph");
  ...
  const handleStaffFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formData.append("permitType", "Staff Official Dispatch");
  ...
  const sendMessage = (e?: React.FormEvent, cannedText?: string) => {
    ...
    if (!textToSend && !staffAttachedFile) return;
    ...
    if (staffAttachedFile) {
      taggedContent = taggedContent 
        ? `${taggedContent} [Attachment: ${staffAttachedFile.name}|${staffAttachedFile.url}]`
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        ...
                        <div>
                          <div style={{ fontSize: "0.86rem", fontWeight: "800", color: hasReceipt ? "#166534" : "#92400e" }}>
                            Payment Assessment: PHP {((relevantApp as any).assessedFees || 3795).toLocaleString()} · {(relevantApp as any).orderOfPaymentNo || "OP-2026"} ({relevantApp.id})
                          </div>
